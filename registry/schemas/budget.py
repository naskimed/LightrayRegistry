...
initial_budget = 30 · floors in slots-REMAINING · alarm_remaining = 15 · diagnostic_remaining = 1.
  [amended 2026-07-09, human decision: 10 -> 30. alarm scaled 5 -> 15 to preserve the
   half-consumed early-warning semantics; diagnostic_remaining stays 1 (reserve the final slot,
   a fixed reserve not a fraction). This is a constants.amend-class change (human-only,
   REG-INV-22); it takes effect when the lineage's look_budget doc is registered at t=0 seed.]
...
class LookBudget(BaseModel):
    ...
    generation: int = 1
    initial_budget: int                      # gen-1 seeded value: 30 (was 10)  ← cite, do not restate
    alarm_remaining: int                     # 15 (floor fires when slots_remaining <= value)
